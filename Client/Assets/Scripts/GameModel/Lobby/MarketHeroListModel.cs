...
namespace BlGame.Model
{
    public class MarketHeroListModel : Singleton<MarketHeroListModel>
    {
        ...
        public void HeroListInfo(GSToGC.GoodsCfgInfo cfgInfo)
        {
            ...
            if (cfg == GSToGC.GoodsCfgInfo.CfgType.CfgTypeDiscount)
            {
                mGoodInfo.DisCountConsume = cfgInfo.Consumes;
                foreach (var item in cfgInfo.Consumes)
                {
            ...
            else if (cfg == GSToGC.GoodsCfgInfo.CfgType.CfgTypeCommon)
            {
                mGoodInfo.CommonConsume = cfgInfo.Consumes;
                foreach (var item in cfgInfo.Consumes)
                {
                    if ((GameDefine.ConsumeType)item.Consumetype == GameDefine.ConsumeType.TypeGold)
                    {
