    ...
    public class MarketRuneListModel : Singleton<MarketRuneListModel>
    {
        ...
        public void AddRuneCfgListInfo(GSToGC.GoodsCfgInfo cfgInfo)
        {
            ...
            if (!RunecfgListDict.TryGetValue(gdid, out mGoodInfo))
            {
                ...
                for (int i = 0; i < cfgInfo.Consumes.Count; ++i )
                {
                    if (cfgInfo.Consumes[i].Consumetype == 2)
                    {
                        mGoodInfo.RLDiamondPrice = cfgInfo.Consumes[i].Price;
                    }
                    else
                    {
                        mGoodInfo.RLGoldPrice = cfgInfo.Consumes[i].Price;
                    }
                }
